...
import MemberRegistration, { RegistrationStatus, AppliedRole, } from "../models/MemberRegistration.js";
import { sendMembershipApplicationEmail, sendMembershipStatusEmail, sendUserCredentialsEmail, } from "../utils/emailHelper.js";
...
export const getFormById = async (req, res) => {
    try {
        const form = await Form.findOne({ formId: req.params.formId }).populate("createdBy", "name email");
        if (!form) {
...
export const createForm = async (req, res) => {
    try {
        const { formId, formTitle, formDescription, requireSunwayEmail, fields } = req.body;
        ...
        const isMembershipForm = formId.toLowerCase().includes("member") ||
...
export const updateForm = async (req, res) => {
    try {
        const { formTitle, formDescription, requireSunwayEmail, fields, isActive } = req.body;
        ...
        if (req.user.role === UserRole.MODERATOR &&
            ...
            return res.status(403).json({
                message: "Moderators are not authorized to update the registration form",
            });
        }
        // Update fields
        if (formTitle)
...
// @access  Private (Admin only)
export const hardDeleteForm = async (req, res) => {
    try {
        const form = await Form.findOne({ formId: req.params.formId });
        if (!form) {
            return res.status(404).json({ message: "Form not found" });
...
export const submitForm = async (req, res) => {
    try {
        ...
        const isMembershipSubmission = membershipFormIds.includes(req.params.formId);
        if (isMembershipSubmission) {
            ...
            const appliedRole = req.params.formId === "general-member-registration"
            ...
            if (existingMember) {
                return res.status(400).json({
                    message: "You have already submitted a membership application with this email address or college ID.",
                });
            }
        ...
        if (req.body.email) {
            ...
            if (existingSubmission) {
                return res.status(400).json({
                    message: "You have already submitted a form for this event with this email address.",
                });
            }
        }
...
export const getFormSubmissions = async (req, res) => {
    try {
        ...
        if (req.user.role === UserRole.MODERATOR &&
            ...
            return res.status(403).json({
                message: "Moderators are not authorized to access registration form submissions",
            });
        }
...
export const updateSubmissionStatus = async (req, res) => {
    try {
        ...
        if (req.user.role === UserRole.MODERATOR &&
            submission.form.formId === "registration") {
            return res.status(403).json({
                message: "Moderators are not authorized to update registration form submissions",
            });
        }
        ...
        if (status === SubmissionStatus.APPROVED && createTeam) {
            ...
            console.log("[Team Creation] Form data:", JSON.stringify(formData, null, 2));
            ...
            if (formData.members && Array.isArray(formData.members)) {
                ...
                if (nameField)
                    memberEntry.name = nameField;
                // Try to find email field
                const emailField = formData.email || formData.team_email || formData.contact_email;
                if (emailField)
                    memberEntry.email = emailField;
                // Try to find phone field
                const phoneField = formData.phone ||
                    formData.contact ||
                    formData.phone_number ||
                    formData.mobile;
                if (phoneField)
                    memberEntry.phone = phoneField;
                // Only add if we have at least name and email
                if (memberEntry.name && memberEntry.email) {
                    members.push(memberEntry);
                }
            }
            console.log("[Team Creation] Extracted members:", JSON.stringify(members, null, 2));
            // Create team from submission data ONLY if a matching event exists
            const eventSlug = submission.form.formId;
            const event = await Event.findOne({ slug: eventSlug });
            if (event) {
                const team = await Team.create({
                    name: formData.team_name || formData.full_name || formData.name || "Team",
                    sport: submission.form.formTitle
                        .replace(/Registration/gi, "")
                        .replace(/Form/gi, "")
                        .replace(/Enrollment/gi, "")
                        .replace(/\s\s+/g, " ")
                        .trim(),
                    teamType: submission.form.formId === "registration"
                        ? TeamType.MEMBER
                        : TeamType.EVENT,
                ...
                console.log("[Team Creation] Skipping team creation: No matching event found for formId:", eventSlug);
            }
            await submission.populate("reviewedBy", "name email");
...
// @access  Private (Admin only)
export const getMemberRegistrations = async (req, res) => {
    try {
        const { status } = req.query;
...
const processRegistrationStatusUpdate = async (id, status, note, adminId) => {
    ...
    if (registration.status !== RegistrationStatus.PENDING) {
        throw new Error(`Registration ${id} is already ${registration.status} and cannot be updated.`);
    }
    ...
    if (status === RegistrationStatus.APPROVED) {
        ...
        if (!user) {
            user = await User.create({
                ...
                role: registration.appliedRole,
            });
        }
        else {
            user.role = registration.appliedRole;
            user.password = hashedPassword;
            await user.save();
        }
        await sendUserCredentialsEmail(registration.email, generatedPassword, registration.name, registration.appliedRole);
        ...
        try {
            ...
            if (!list.emails.includes(registration.email)) {
                ...
                await list.save();
                console.log(`[NotificationList] Subscribed approved user: ${registration.email}`);
            }
        }
        catch (notifError) {
    ...
    await sendMembershipStatusEmail(registration.email, registration.name, status, registration.appliedRole, note);
    return registration;
};
// @desc    Update membership registration status
// @route   PATCH /api/forms/membership-registrations/:id/status
// @access  Private (Admin only)
export const updateMemberRegistrationStatus = async (req, res) => {
    try {
        const { status, note } = req.body;
        const registration = await processRegistrationStatusUpdate(req.params.id, status, note, String(req.user?._id));
...
// @access  Private (Admin only)
export const bulkUpdateMemberRegistrationStatus = async (req, res) => {
    try {
        ...
        for (const id of ids) {
            try {
                const registration = await processRegistrationStatusUpdate(id, status, note, String(req.user?._id));
