...
// @access  Public
export const getDrawsByEvent = async (req, res) => {
    try {
...
// @access  Private (Admin)
export const updateDrawResults = async (req, res) => {
    try {
        ...
        if (matchResults) {
            Object.entries(matchResults).forEach(([matchId, winnerId]) => {
                draw.matchResults.set(matchId, winnerId);
            });
        }
        await draw.save();
    ...
    try {
        ...
        if (matchScores) {
            Object.entries(matchScores).forEach(([matchId, score]) => {
                draw.matchScores.set(matchId, score);
            });
        }
        await draw.save();
        // Populate drawnTeams before returning
