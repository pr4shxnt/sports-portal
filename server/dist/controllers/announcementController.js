...
const transformAnnouncement = (ann, userId) => {
    const obj = ann.toObject ? ann.toObject() : ann;
    const reactionsSummary = ann.reactions
        ? ann.reactions.map((r) => ({
            emoji: r.emoji,
            count: r.users.length,
            isReacted: r.users.some((id) => id.toString() === userId?.toString()),
        }))
        : [];
    return {
        ...obj,
        ...
        comments: obj.comments.map((c) => ({
            ...
            isMe: c.userId?._id?.toString() === userId?.toString() ||
...
export const getAnnouncements = async (req, res) => {
    try {
        ...
        const announcements = await Announcement.find({})
            ...
            .populate("author", "name role")
            .populate("comments.userId", "name role");
        const transformed = announcements.map((ann) => transformAnnouncement(ann, userId));
        ...
        (async () => {
            try {
                ...
                if (list && list.emails.length > 0) {
                    ...
                    const users = await User.find({ email: { $in: list.emails } }).select("email name");
...
export const toggleReaction = async (req, res) => {
    try {
        ...
        announcement.reactions.forEach((r, idx) => {
            const uIdx = r.users.findIndex((uid) => uid.toString() === userId?.toString());
            if (uIdx > -1) {
        ...
        if (existingEmojiIndex > -1) {
            ...
            announcement.reactions[existingEmojiIndex].users.splice(existingUserIndexInEmoji, 1);
        ...
        const targetReactionIndex = announcement.reactions.findIndex((r) => r.emoji === emoji);
        if (targetReactionIndex > -1) {
            announcement.reactions[targetReactionIndex].users.push(userId);
        }
        else {
