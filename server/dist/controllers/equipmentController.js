    ...
    try {
        ...
        if (currentHourNPT < 9 || currentHourNPT >= 17) {
            return res.status(403).json({
                message: "Equipment requests are only allowed between 9:00 AM and 5:00 PM daily (Nepal Time).",
            });
        }
...
export const updateResponsibilityStatus = async (req, res) => {
    try {
...
// @access  Admin, Superuser
export const forceReturn = async (req, res) => {
    try {
        const resp = await Responsibility.findById(req.params.id);
        if (!resp)
...
// @access  Admin, Superuser
export const getChainOfCustodyReport = async (req, res) => {
    try {
