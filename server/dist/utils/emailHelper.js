...
export const getTransporter = () => {
    ...
    if (!user || !pass) {
        console.error("CRITICAL: EMAIL_USER or EMAIL_PASS not found in environment variables");
...
export const getOfficialTransporter = () => {
    ...
    if (!user || !pass) {
        console.error("CRITICAL: EMAIL_USER_OFFICIAL or EMAIL_PASS_OFFICIAL not found in environment variables");
...
export const sendUserCredentialsEmail = async (email, password, name, role) => {
    const roleLabels = {
        admin: "Executive",
        superuser: "Staff",
...
export const sendMembershipApplicationEmail = async (email, name, appliedRole) => {
    const roleLabel = appliedRole === "moderator"
...
export const sendMembershipStatusEmail = async (email, name, status, appliedRole, note) => {
...
export const sendMeetingInvitationEmail = async (params) => {
    const { createEvent } = await import("ics");
    const { to, recipientName, title, topic, date, time, location, meetingLink, type, venue, roomNo, } = params;
    ...
    if (type === "virtual" && meetingLink) {
        ...
        locationLabel = venue || "";
        if (roomNo)
            locationLabel += `, Room ${roomNo}`;
        locationHtml = locationLabel;
    }
    // Build Join button for virtual meetings
    const joinButton = type === "virtual" && meetingLink
        ? `
    ...
    const icsResult = createEvent({
        ...
        location: type === "virtual" ? meetingLink || "" : locationLabel || location,
...
export const sendAnnouncementEmail = async (email, userName, title, content) => {
    const mailOptions = {
        from: `"Sports Club Portal" <${process.env.EMAIL_USER}>`,
        to: email,
